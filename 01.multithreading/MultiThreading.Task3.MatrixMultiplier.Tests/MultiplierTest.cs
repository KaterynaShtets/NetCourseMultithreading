...
namespace MultiThreading.Task3.MatrixMultiplier.Tests
{
    ...
    public class MultiplierTest
    {
        ...
        public void ParallelEfficiencyTest()
        {
            ...
            sw.Stop();
            var notParallelExecutionTime = sw.ElapsedMilliseconds;
            sw.Reset();
            sw.Start();
            parallelMatricesMultiplier.Multiply(firstMatrix, secondMatrix);
            sw.Stop();
            var parallelExecutionTime = sw.ElapsedMilliseconds;

            Assert.IsTrue(parallelExecutionTime < notParallelExecutionTime);
        }
